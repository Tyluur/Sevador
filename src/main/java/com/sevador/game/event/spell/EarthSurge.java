...
/**
 * Handles the Earth surge spell event.
 * @author Emperor
 *
 */
public class EarthSurge implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 7), new Item(557, 10), new Item(560, 1), new Item(565, 1)};
	
	/**
	 * The start animation.
	 */
	private static final Animation ANIMATION = new Animation(14209, 0, false, Priority.HIGH);
	
	/**
	 * The start graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(2717, 0, 0, false);
	
	@Override
	public boolean init() {
		return EventManager.register(89, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2727, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2722, 38, 32, 54, speed, 0, 124));
