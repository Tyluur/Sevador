...
/**
 * Handles the Miasmic barrage spell event.
 * 
 * @author Emperor
 * 
 */
public final class MiasmicBarrage implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(566, 4),
			new Item(565, 4), new Item(557, 4) };

	/**
	 * The start animation.
	 */
	private static final Animation ANIMATION = new Animation(10518, 0, false,
			Priority.HIGH);

	/**
	 * The start graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(1853, 0, 0, false);

	@Override
	public boolean init() {
		return EventManager.register(65575, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		if (weaponId != 13867 && weaponId != 13869 && weaponId != 13941
			...
			p.getPacketSender().sendMessage(
					"You need Zuriel's staff to cast this spell.");
			return false;
		}
		i.start = ANIMATION;
		...
		i.targets.get(0).projectile.add(Projectile.create(p, i.victim, 366, 43,
				0, 51, 120, 16, 64));
		...
		for (Target t : i.targets) {
			...
			t.graphic = new Graphic(1854, 0, 0, t.entity.isNPC());
			...
			if (hit > 0
					&& t.entity.getAttribute("miasmicImmunity", -1) < MajorUpdateWorker
							.getTicks()) {
				...
				t.entity.setAttribute("miasmicTime",
						MajorUpdateWorker.getTicks() + 80);
				t.entity.setAttribute("miasmicImmunity",
						MajorUpdateWorker.getTicks() + 95);
			}
		}
		return true;
	}
