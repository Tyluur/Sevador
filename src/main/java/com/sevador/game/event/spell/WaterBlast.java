...
/**
 * Handles the Water blast spell event.
 * 
 * @author Emperor
 * 
 */
public final class WaterBlast implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(556, 3),
			new Item(555, 3), new Item(560, 1) };
	...
	@Override
	public boolean init() {
		return EventManager.register(52, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2706, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(
				i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2705, 28, 32,
				55, speed, 3, 161));
