...
/**
 * Handles the Blood burst spell event.
 * 
 * @author Emperor
 * 
 */
public final class BloodBurst implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(565, 2),
	...
	@Override
	public boolean init() {
		return EventManager.register(65562, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		for (Target t : i.targets) {
			...
			t.graphic = new Graphic(376, 0, 0, t.entity.isNPC());
			...
			if (hit > 0) {
				if (t.entity.isPlayer()) {
					t.entity.getPlayer().getPacketSender()
							.sendMessage("Your lifepoints have been drained.");
				}
