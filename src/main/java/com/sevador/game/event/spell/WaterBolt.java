...
/**
 * Handles the Water bolt spell event.
 * 
 * @author Emperor
 * 
 */
public final class WaterBolt implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(556, 2),
			new Item(555, 2), new Item(562, 1) };

	/**
	 * The start animation.
	 */
	private static final Animation ANIMATION = new Animation(10542, 0, false,
			Priority.HIGH);
	...
	@Override
	public boolean init() {
		return EventManager.register(39, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2709, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(
				i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2704, 28, 32,
				55, speed, 3, 161));
