...
/**
 * Represents the smoke barrage spell event.
 * @author Emperor
 *
 */
public class SmokeBarrage implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 4), new Item(554, 4), new Item(565, 2), new Item(560, 4)};
	...
	@Override
	public boolean init() {
		return EventManager.register(65567, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		i.targets.get(0).projectile.add(Projectile.create(i.source, i.victim, 390, 43, 31, 51, speed, 16, 64));
		for (Target t : i.targets) {
			...
			t.graphic = new Graphic(391, 0, 0, t.entity.isNPC());
