...
/**
 * Handles the Earth bolt spell event.
 * 
 * @author Emperor
 * 
 */
public final class EarthBolt implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(556, 2),
			new Item(557, 3), new Item(562, 1) };
	 ...
	 * The start graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(2714, 0, 0, false);

	@Override
	public boolean init() {
		return EventManager.register(42, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2724, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(
				i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2719, 38, 32,
				54, speed, 0, 124));
