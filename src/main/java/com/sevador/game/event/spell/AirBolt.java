...
/**
 * Handles the Air bolt spell event.
 * @author Emperor
 *
 */
public final class AirBolt implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 2), new Item(562, 1)};
	...
	@Override
	public boolean init() {
		return EventManager.register(34, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(464, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 459, 22, 32, 60, speed, 0, 90));
	...
	public int getStartDamage(Entity e, Entity victim) {
		return e.getPlayer().getEquipment().getItem(Constants.SLOT_HANDS).getId() == 777 ? 
