...
/**
 * Handles the Fire wave spell event.
 * @author Emperor
 *
 */
public class FireWave implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 5), new Item(554, 7), new Item(565, 1)};
	
	/**
	 * The start animation.
	 */
	private static final Animation ANIMATION = new Animation(2791, 0, false, Priority.HIGH);
	...
	@Override
	public boolean init() {
		return EventManager.register(80, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2740, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2733, 30, 32, 52, speed, 3, 150));
		t.projectile.add(Projectile.create(i.source, i.victim, 2734, 30, 32, 52, speed, 20, 150));
		t.projectile.add(Projectile.create(i.source, i.victim, 2734, 30, 32, 52, speed, 110, 150));
