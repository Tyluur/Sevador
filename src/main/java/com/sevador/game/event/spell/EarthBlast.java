...
/**
 * Handles an Earth blast spell event.
 * @author Emperor
 *
 */
public final class EarthBlast implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 3), new Item(557, 4), new Item(560, 1)};
	 ...
	 * The start graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(2715, 0, 0, false);
	
	@Override
	public boolean init() {
		return EventManager.register(58, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2725, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2720, 38, 32, 54, speed, 0, 124));
