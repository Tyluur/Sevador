...
/**
 * Handles the Water surge spell event.
 * 
 * @author Emperor
 * 
 */
public final class WaterSurge implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(556, 7),
			new Item(555, 10), new Item(560, 1), new Item(565, 1) };
	...
	@Override
	public boolean init() {
		return EventManager.register(87, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2712, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(
				i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2707, 28, 32,
				55, speed, 3, 161));
