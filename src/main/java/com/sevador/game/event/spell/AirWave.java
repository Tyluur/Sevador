...
/**
 * Handles the Air wave spell effect.
 * @author Emperor
 *
 */
public final class AirWave implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	
	private static final Item[] RUNES = new Item[] {new Item(556, 5), new Item(565, 1)};
	...
	@Override
	public boolean init() {
		return EventManager.register(70, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2699, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 461, 22, 32, 60, speed, 0, 90));
