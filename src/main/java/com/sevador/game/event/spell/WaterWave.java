...
/**
 * Handles the Water wave spell event.
 * @author Emperor
 *
 */
public final class WaterWave implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] {new Item(556, 5), new Item(555, 7), new Item(565, 1)};
	
	/**
	 * The start animation.
	 */
	private static final Animation ANIMATION = new Animation(10542, 0, false, Priority.HIGH);
	
	/**
	 * The start graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(2701, 0, 0, false);
	
	@Override
	public boolean init() {
		return EventManager.register(73, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2711, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2706, 28, 32, 55, speed, 3, 161));
