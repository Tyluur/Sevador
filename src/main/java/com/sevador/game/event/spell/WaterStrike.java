 ...
 * 
 */
public class WaterStrike implements MagicSpell {

	/**
	 * The runes required to cast this spell.
	 */
	private static final Item[] RUNES = new Item[] { new Item(556, 1), new Item(555, 1), new Item(558, 1) };
	...
	@Override
	public boolean init() {
		return EventManager.register(28, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.graphic = new Graphic(2708, 96, 0, i.victim.isNPC());
		int speed = (int) (46 + i.source.getLocation().distance(
				i.victim.getLocation()) * 10);
		t.projectile.add(Projectile.create(i.source, i.victim, 2703, 22, 32, 60,
				speed, 0, 90));
