...
/**
 * Handles the Statius' warhammer special attack event.
 * @author Emperor
 *
 */
public final class Smash implements TypeHandler {
	
	/**
	 * The amount of special attack energy required.
	 */
	private static final int ENERGY_REQUIRED = 35;
	
	/**
	 * The animation.
	 */
	private static final Animation ANIMATION = new Animation(10505, 0, false, Priority.HIGHEST);
	
	/**
	 * The graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(1840, 0, 0, false);
	
	@Override
	public boolean init() {
		EventManager.register(13902, this);
		return EventManager.register(13904, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		t.entity.getSkills().updateLevel(Skills.DEFENCE, (int) -(1 + t.entity.getSkills().getLevel(Skills.DEFENCE) * 0.3), 0);
		i.targets.add(t);
		return true;
	}
	...
	public double getAccuracy(Entity e, Object... args) {
		return CombatType.MELEE.getHandler().getAccuracy(e, args) * 1.092;
	}

	@Override
	public double getMaximum(Entity e, Object... args) {
		return CombatType.MELEE.getHandler().getMaximum(e, args) * 1.25;
	}

	@Override
	public double getDefence(Entity e, int attackBonus, Object... args) {
		return CombatType.MELEE.getHandler().getDefence(e, attackBonus, args) * 0.97;
	}
