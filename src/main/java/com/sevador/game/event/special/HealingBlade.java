...
/**
 * Handles the Saradomin godsword special attack event.
 * @author Emperor
 *
 */
public final class HealingBlade implements TypeHandler {
	 ...
	 * The animation.
	 */
	private static final Animation ANIMATION = new Animation(12019, 0, false, Priority.HIGHEST);
	
	/**
	 * The graphic.
	 */
	private static final Graphic GRAPHIC = new Graphic(2109, 0, 0, false);
	
	@Override
	public boolean init() {
		EventManager.register(11698, this);
		return EventManager.register(13452, this);
	}
	...
	public boolean handle(Interaction i) {
		...
		i.targets.add(t);
		int toHeal = t.damage.getHit() / 2;
		double toRestore = t.damage.getHit() * 0.25;
		if (toHeal < 100) {
			toHeal = 100;
		}
		if (toRestore < 5) {
		...
		i.source.getSkills().heal(toHeal);
		i.source.getSkills().updatePrayerPoints(-toRestore);
		return true;
	}

	@Override
	public double getAccuracy(Entity e, Object... args) {
		return CombatType.MELEE.getHandler().getAccuracy(e, args) * 1.12;
	}
