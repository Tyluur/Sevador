...
public final class RangeWeapon {
	...
	public static boolean initialize() {
		...
		try {
			...
			doc = builder.parse(new File("./data/xml/range_weapon_data.xml"));
		} catch (Throwable e) {
			e.printStackTrace();
			return false;
		}
		NodeList nodeList = doc.getDocumentElement().getChildNodes();
		Main.getLogger().info("Loading range weapon info...");
		for (short i = 1; i < nodeList.getLength(); i += 2) {
			...
			if (n != null) {
				if (n.getNodeName().equalsIgnoreCase("Weapon")) {
					...
					RANGE_WEAPONS.put(itemId, new RangeWeapon(itemId, new Animation(animationId, 0, false), attackSpeed, slot, type, dropAmmo, ammo));
				}
			}
		}
		Main.getLogger().info("Loaded " + RANGE_WEAPONS.size() + " range weapon definitions.");
		return true;
	}
