...
public final class NPCDefinitions {
	...
	public static final NPCDefinitions getNPCDefinitions(int npcId) {
		NPCDefinitions def = npcDefinitions.get(npcId);
		if (def == null) {
			def = new NPCDefinitions(npcId);
			def.method694();
			try {
				byte[] data = CacheManager
						.getData(CacheConstants.NPCDEF_IDX_ID, npcId >>> 7,
								npcId & 0x7f);
				if (data == null)
					System.out.println("Failed loading NPC " + npcId + ".");
				else
					def.readValueLoop(new RSInputStream(
